...
namespace ImageTest05
{
    ...
    {
        private void Create_ImageFFTArray(){
...
*/
            ...
            // 出力path
//            outputFPath = @"C:\Image\NewImage\FFTtest.png";
//            if(output == true){
            // 出力BitmapSource(outputBitmap)を作る
                outputFBitmap = BitmapSource.Create(width, height, 96, 96, PixelFormats.Pbgra32, null, colorF, stride);

            // BitmapSourceを保存する
/*                using (Stream stream = new FileStream(outputFPath, FileMode.Create))
                {
